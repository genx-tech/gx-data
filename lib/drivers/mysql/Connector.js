...
class MySQLConnector extends Connector {
  ...
  constructor(connectionString, options) {
    ...
    this.$call = (name, alias, args, extra) => ({ ...extra,
      type: 'function',
      name,
      alias,
      args
  ...
  _buildQuery(model, {
    ...
    const selectColomns = $projection ? this._buildColumns($projection, params, hasJoining, aliasMap) : '*';
    ...
    if ($groupBy) {
      groupByClause += ' ' + this._buildGroupBy($groupBy, params, hasJoining, aliasMap);
    }
  ...
  _buildQueryWithTotalCount(model, {
    $relationships,
    $projection,
    $query,
    $groupBy,
    ...
    if (typeof $totalCount !== 'string') {
      throw new InvalidArgument('"$totalCount" should be a field name used to count the record.');
    }
    ...
    if ($orderBy) {
      orderByClause += ' ' + this._buildOrderBy($orderBy, hasJoining, aliasMap, requiredColumns);
    }
    ...
    const distinctField = this._escapeIdWithAlias($totalCount, hasJoining, aliasMap);
    ...
    this._joinAssociation({
      ...
      output: true
    }, keySqlAnchor, joinings, model, aliasMap, keySqlAlias, joiningParams);

    fromAndJoin = fromClause + joinings.join(' ');
    result.sql = withTables + 'SELECT ' + selectColomns + fromAndJoin + whereClause;
  ...
  _buildOrderBy(orderBy, hasJoining, aliasMap, requiredColumns) {
    if (typeof orderBy === 'string') {
      return 'ORDER BY ' + this._buildOrderByColumn(orderBy, hasJoining, aliasMap, requiredColumns);
    }

    if (Array.isArray(orderBy)) return 'ORDER BY ' + orderBy.map(by => this._buildOrderByColumn(by, hasJoining, aliasMap, requiredColumns)).join(', ');

    if (_.isPlainObject(orderBy)) {
      return 'ORDER BY ' + _.map(orderBy, (asc, col) => this._buildOrderByColumn(col, hasJoining, aliasMap, requiredColumns) + (asc === false || asc === -1 ? ' DESC' : '')).join(', ');
    }
