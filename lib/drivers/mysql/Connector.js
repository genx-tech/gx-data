...
class MySQLConnector extends Connector {
  ...
  async find_(model, condition, connOptions) {
    ...
    if (sqlInfo.countSql) {
      const [countResult] = await this.execute_(sqlInfo.countSql, sqlInfo.params, connOptions);
    ...
    if (sqlInfo.hasJoining) {
      ...
      const reverseAliasMap = _.reduce(sqlInfo.aliasMap, (result, alias, nodePath) => {
  ...
  _joinAssociations(associations, parentAliasKey, parentAlias, aliasMap, startId, params) {
    ...
    _.each(associations, (assocInfo, anchor) => {
      ...
      if (subAssocs) {
        const subJoinings = this._joinAssociations(subAssocs, aliasKey, alias, aliasMap, startId, params);
