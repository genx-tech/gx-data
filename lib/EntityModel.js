    ...
    const success = await this._safeExecute_(async context => {
      ...
      if (!context.options.$dryRun) {
        if (context.options.$upsert) {
          ...
          const dataForInserting = typeof context.options.$upsert === 'object' ? { ...context.latest,
            ...context.options.$upsert
          } : context.latest;
          context.result = await this.db.connector.upsertOne_(this.meta.name, dataForUpdating, this.getUniqueKeyFieldsFrom(context.latest), context.connOptions, dataForInserting);
        } else {
